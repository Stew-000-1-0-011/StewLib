// 定数式ではplacement newもpseudo destructorも使えない。よって無用の長物である
// (std::arrayに比べると例外安全性が明確に定まっていたり、fillが並列実行可能ではあるが...)

#pragma once

#include "static_warn.hpp"
Stew_static_warn(false, "This class cannot use in constexpr. So, almost useless.");
...
namespace StewLib
{
    ...
    struct Array final
    {
        ...
        Uninitialized<T> elements[n]{};

        constexpr Array() noexcept(std::is_nothrow_default_constructible_v<T>)
        {
        ...
        constexpr Array(const T(&elements)[n]) noexcept(std::is_nothrow_copy_constructible_v<T>)
        {
            for(std::size_t i = 0; i < n; ++i)
            {
                new(&this->elements[i].value) T{elements[i]};
            }
        }

        constexpr Array(T(&&elements)[n]) noexcept(std::is_nothrow_move_constructible_v<T>)
        {
            for(std::size_t i = 0; i < n; ++i)
            {
                new(&this->elements[i].value) T{std::move(elements[i])};
            }
        }
